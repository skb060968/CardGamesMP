...
export function createFirebaseRoomStore({
      ...
      result = await transactExistingRoom('join-room', (currentPlayers) => {
        ...
        for (let index = 0; index < SLOT_COUNT; index += 1) {
          ...
          const profileChanged = Object.entries(safePlayer)
            .some(([key, value]) => !sameValue(existing[key], value));
          if (profileChanged) {
            rejectionCode = 'identity-already-in-room';
            return undefined;
          }
          joinedIndex = index;
          idempotent = true;
  ...
  async function commitBluffAction(payload = {}) {
    try {
      requireObject(payload, 'Bluff commit payload');
      const {
        moveId, expectedRevision, playerIndex, action: actionType,
        payload: actionPayload, state,
      } = payload;
      requireToken(moveId, 'moveId');
      if (!Number.isSafeInteger(expectedRevision) || expectedRevision < 0) {
        throw new TypeError('expectedRevision must be a non-negative safe integer');
      ...
      const isExactPayload = (value, keys) => value !== null
        && typeof value === 'object'
        && !Array.isArray(value)
        && Object.keys(value).length === keys.length
        && Object.keys(value).every((key) => keys.includes(key));
      if (actionType === 'place') {
        if (!isExactPayload(actionPayload, ['cardIds', 'declaredRank'])
          || !Array.isArray(actionPayload.cardIds)
          || actionPayload.cardIds.length < 1 || actionPayload.cardIds.length > 4
          || actionPayload.cardIds.some((id) => typeof id !== 'string' || !id)
          || new Set(actionPayload.cardIds).size !== actionPayload.cardIds.length
        ...
        if (!isExactPayload(actionPayload, ['placementMoveId', 'placementRevision'])
          ...
          || !Number.isSafeInteger(actionPayload.placementRevision)
          || actionPayload.placementRevision < 1) {
          throw new TypeError(`Invalid Bluff ${actionType} payload`);
        }
      } else if (actionPayload !== null) {
        throw new TypeError('Bluff pass payload must be null');
      }
      ...
      const safePayload = actionPayload === null
        ? null
        : cloneFirebaseValue(actionPayload, 'Bluff action payload');
      if (nextState.revision !== expectedRevision + 1) {
        throw new RoomCommitError('invalid-next-revision');
      }
      ...
      let rejectionCode = 'transaction-aborted';
      let idempotent = false;
      const result = await transactExistingRoom('commit-bluff-action', (currentRoom) => {
        rejectionCode = 'transaction-aborted';
        idempotent = false;
        if (!currentRoom) { rejectionCode = 'room-not-found'; return undefined; }
        ...
        const previousMove = currentRoom.lastMove;
        if (previousMove?.id === moveId) {
          const samePayload = safePayload === null
            ? !Object.prototype.hasOwnProperty.call(previousMove, 'payload')
            : sameValue(previousMove.payload, safePayload);
          const sameMove = previousMove.type === 'bluff-action'
            && previousMove.expectedRevision === expectedRevision
            && previousMove.revision === nextState.revision
            && previousMove.playerIndex === playerIndex
            && previousMove.action === actionType
        ...
        return {
          ...
          game: encodedNextState,
          lastMove,
          meta: { ...currentRoom.meta, lastActivity: commitTime },
        };
      });

      if (!result.committed) {
        throw new RoomCommitError(rejectionCode, `Bluff commit rejected: ${rejectionCode}`);
      }
      const committedRoom = result.snapshot.val();
      if (!committedRoom?.game) throw new RoomCommitError('missing-committed-state');
      return {
      ...
      throw commitError(error, 'Bluff commit');
    }
  }
