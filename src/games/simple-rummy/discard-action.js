...
export function createSimpleRummyDiscardAction({
      ...
      steps: [({ signal }) => executeCommittedAction({
        ...
        prepare: () => {
          ...
          return {
            ...
            winGroups: result.winGroups ?? null, expectedRevision,
          };
        },
        commit: (prepared) => sync.commitDiscard({
          moveId, expectedRevision: prepared.expectedRevision, playerIndex, handIndex,
          card: prepared.card, won: prepared.won, winGroups: prepared.winGroups,
        ...
        publish: async (prepared, committed) => {
          ...
          await effects.render({
            moveId, playerIndex, handIndex, card: prepared.card,
            won: prepared.won, winGroups: prepared.winGroups, state: authoritativeState,
          });
        },
      })],
    });
