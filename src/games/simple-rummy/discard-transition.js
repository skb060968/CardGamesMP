...
export function createSimpleRummyDiscardTransitionValidator(rules) {
  ...
  return ({ currentState, nextState, action } = {}) => {
    ...
    try {
      ...
      if ('winGroups' in action && !sameValue(action.winGroups, result.winGroups ?? null)) {
        return { valid: false, reason: 'win-groups-mismatch' };
      }
